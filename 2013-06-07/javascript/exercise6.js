...
//in uscita è restituito il testo, una stringa che rappresenta il contenuto del file obj.
...
function lar_to_obj(V,FV){
	...
	for(i = 0; i < V.length; i++){
		result = result + "point" + ": (";
		for(coordinata = 0; coordinata < V[i].length; coordinata++){
	...
	for(i = 0; i < FV.length; i++){
		result = result + "facet" + ": (";
		for(vertex = 0; vertex < FV[i].length; vertex++){
